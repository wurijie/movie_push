...
class dytt():
    def __init__(self):
        self.index_url = "https://www.dytt8.net/index.htm"
    ...
    def request_tool(self):
        try:
            r = requests.get(url=self.index_url, headers=self.headers, timeout=15)
            r.encoding = "gb2312"
    ...
    def getMovies(self):
        ...
        for td in target_tds:
            ...
            if len(td_a)>1:
                name = td_a[1].text
                matched_name = re.search(r"《(.+)》", name)
                if not matched_name:
                    continue
                else:
                    name = matched_name.groups()[0]
